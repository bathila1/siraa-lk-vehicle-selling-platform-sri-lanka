...
export default async function PostAdPage() {
  ...
  const [{ data: attrSchema }, { data: promoSetting }, { data: cities }] =
    await Promise.all([
      supabase
        .from('vehicle_attributes_schema')
        .select('*')
        .eq('active', true)
        .order('sort_order'),
      supabase
      ...
      supabase
        .from('cities')
        .select('id, district_id, name_en')
        .order('sort_order'),
    ]);

  const promoActive =
    promoSetting?.value?.active === true &&
    (promoSetting?.value?.current_count ?? 0) <
      (promoSetting?.value?.max_count ?? 100);
